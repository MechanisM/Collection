...
0){c.page="-=1";this.print(c,true)}else this.easyPage(c);return this};d.Collection.fn.easyPage=function(a){var b="",e=a.nmbOfEntries%a.numberBreak!==0?~~(a.nmbOfEntries/a.numberBreak)+1:a.nmbOfEntries/a.numberBreak,c=function(h,l,m){var n,o="<"+(h.tag||"span")+' data-page="'+m+'"';if(h.attr)for(n in h.attr)if(h.attr.hasOwnProperty(n))o+=" "+n+'="'+h.attr[n]+'"';if(m===a.page)o+=' class="'+(l&&l.active||"active")+'"';return o+(">"+m+"</"+(h.tag||"span")+">")},g,f=0,j,k;a.pager.find("[data-ctm]").each(function(){var h=
d(this),l=h.data("ctm"),m=l.classes;if(l.nav){if(l.nav==="prev"&&a.page===1||l.nav==="next"&&a.finNumber===a.nmbOfEntries)h.addClass(m&&m.disabled||"disabled");else if(l.nav==="prev"||l.nav==="next")h.removeClass(m&&m.disabled||"disabled");if(l.nav==="pageList"){if(e>a.pageBreak){if(a.page===1)j=0;else if(a.pageBreak%2!==0){k=j=(a.pageBreak-1)/2;if(a.page-1<j)j=0;else{j=a.page-j-1;if(a.page+k>e)j-=a.page+k-e}}g=j;for(f=-1;(g+=1)<=e&&(f+=1)!==null;){if(f===a.pageBreak&&g!==a.page)break;b+=c(l,m||"",
g)}}else for(g=0;(g+=1)<=e;)b+=c(l,m||"",g);h.html(b)}}else if(l.info){a.nmbOfEntriesInPage===0?h.addClass(m&&m.noData||"noData"):h.removeClass(m&&m.noData||"noData");switch(l.info){case "page":h.html(a.page);break;case "total":h.html(a.nmbOfEntries);break;case "from":h.html((a.page-1)*a.numberBreak+1);break;case "to":h.html(a.finNumber);break;case "inPage":h.html(a.nmbOfEntriesInPage);break;case "nmbOfPages":h.html(a.nmbOfPages)}}});return this};d.Collection.fn.genTable=function(a,b,e){a=a||4;b=
b||"div";e=e===false?false:true;var c=1,g,f=this.dObj.active.target,j=f.find(b).length,k="";f.find(b).each(function(h){this.tagName!=="td"&&d(this).wrap("<td></td>");if(c===a){k="";for(g=-1;(g+=1)<a;){k+="td:eq("+(h-g)+")";if(g!==a-1)k+=","}f.find(k).wrapAll("<tr></tr>");c=0}else if(h===j-1&&c!==a){k="";for(g=-1;(g+=1)<c;){k+="td:eq("+(h-g)+")";if(g!==c-1)k+=","}c-=1;f.find(k).wrapAll("<tr></tr>");if(e===true){for(k="";(c+=1)<a;)k+="<td></td>";f.find("tr:last").append(k)}}c+=1});f[0].tagName!=="table"&&
f.children("tr").wrapAll("<table></table>");return this}})(jQuery);
//