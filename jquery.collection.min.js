...
1;e.collection=c.isString(e.collection)?this._get("collection",e.collection):e.collection;e.template=c.isString(e.template)?this._get("template",e.template):e.template;e.filter=c.isExist(a.filter)&&a.filter!==true?a.filter:this.getActiveParam("filter");e.parser=c.isExist(a.parser)?a.parser:this.getActiveParam("parser");if(d===true)e.cache.iteration=false;d=f.page-e.page;this.updatePage(e.page);m=function(n,o,q,r,s,p){e.callback&&e.callback.apply(this,arguments);l+=e.template.call(e.template,n,o,q,
r,s,p);h=o;return true};a=nimble.byLink(e.collection,this.getActiveParam("context").toString()+nimble.CHILDREN+(a&&a.context||""));g=this.length();e.numberBreak=!b||e.numberBreak===false?g:e.numberBreak;if(c.isPlainObject(a)||e.cache.iteration===false||e.cache.firstIteration===false||e.cache.lastIteration===false){j=!b||e.page===1?0:(e.page-1)*e.numberBreak;this.forEach(m,e.filter,this.ACTIVE,true,e.numberBreak,j);if(e.cache.iteration===false)e.cache.lastIteration=false}else if(c.isArray(a)&&e.cache.iteration===
...
false){f.cache.firstIteration=e.cache.lastIteration;f.cache.lastIteration=h+1}if(e.cache.autoIteration===true)f.cache.iteration=true;l=!l?e.resultNull:this._customParser(e.parser,l);if(e.target===false){e.variable?this._push("variable",e.variable,l):this._new("variable",l);return this}else e.target[e.appendType](l);if(!b)return this;e.nmbOfEntries=e.filter!==false?this.length(e.filter):g;e.nmbOfEntriesInPage=e.target.find(e.calculator).length;e.finNumber=e.numberBreak*e.page-(e.numberBreak-e.nmbOfEntriesInPage);
